...
tracks <- read_csv(file.path(data_dir, csv_tracks)) %>%
  mutate(Track_Speed = Track_Speed * spdscale) %>%
  mutate(Track_Speed_X = Track_Speed_X * spdscale) %>%
  mutate(Track_Speed_Y = Track_Speed_Y * spdscale) %>%
  mutate(Track_Speed_max = Track_Speed_max * spdscale) %>%
...
Please note that the angles given in the normalized data are already rotated! 
...
Results: the migration in HEM shows a higher VOT compared to FN. Do we see a significant difference? Answer: yes, we get p = 0.008 in the data with manually removed frames (not cleaned data p = 0.0216 )
```{r}
...
show a very high standard deviation. Hmm... Lets have a close look at these migrations.
```{r}
population %>% 
  ungroup() %>%
  ...
  summarise(mean_speed = mean(Track_Speed), sd_speed = sd(Track_Speed))
```
Aha, it is gn0055. Lets see which conditions / doses are affected  
```{r}
population %>% 
  ungroup() %>%
  filter(Metadata_condition == 'CP17', Metadata_matrix == 'HEM', Metadata_id == 'gn0055') %>%
  group_by(Metadata_condition, Metadata_dose) %>%
  summarise(mean_speed = mean(Track_Speed), sd_speed = sd(Track_Speed))
```

Only one dose is running wild. We remove it :) 
...
population %<>%
 filter( !((Metadata_id == 'gn0055') & (Metadata_condition == 'CP17') &  (Metadata_dose == '5') ) ) 

```

... and look again. This looks much nicer :)
```{r}
population %>% 
  ungroup() %>% 
  group_by(Metadata_condition,Metadata_matrix ) %>%
...
```
# Normalized data
For the further analysis, we use normalized data. The normalization was performed using cytominer::normalize, see **example_normalize_matlab.Rmd**. Again, we remove the one bad experiment. 
```{r}
...
We calculate the fraction of tracks migrating in each direction. The migration in FN shows 43.6%  neutros migrating towards the chemoattractant (44.2% using all frames).
```{r}
...
Do we see a difference for different conditions? 

Summary: 
1. The migration in HEM does not show such a high variance as in FN (mean 43.6%, sd 15.2%) compard to HEM (mean 31.5%, sd 7.4% )
