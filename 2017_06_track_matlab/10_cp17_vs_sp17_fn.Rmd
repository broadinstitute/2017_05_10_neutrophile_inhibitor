...
```{r}
ttest_dose(feature = "Track_Speed", dose = 5)
```
...
```{r}
ttest_dose(feature = "Track_Directionality", dose = 5
  )
```
### Summary Track_xFMI
...
ttest_dose(feature = "Track_xFMI", dose = 5)
...
The same analysis is repeated for HEM
```{r}
normalized %>% 
  filter(Metadata_condition == "CP17") %>% 
    ggplot(data = .) +
...
```{r}
normalized %>% 
  filter(Metadata_dose != 0.5, Metadata_condition != "control") %>% 
  group_by(Metadata_matrix,Metadata_condition, Metadata_dose, Metadata_id) %>%
  slice(1) %>%
...
```
```{r}
postprocessed <- read_csv(file.path(analysis_folder,'clean_tracks.csv')) %>% 
  group_by_(.dots = strata)
